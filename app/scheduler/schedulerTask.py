...
def task_send_notif_stock_price(data):
    try:
        print("task_send_notif_stock_price: " + data + f" datetime {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}") 
...
def fun_test(data):
    try:
        print("fun2: " + data+  f" datetime {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        stock_code_list = ast.literal_eval(data)
        chatBot.send_message(f"Executing fun2 with stock codes: {stock_code_list} datetime {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    except Exception as e:
        print(f"Error fetching stock prices between dates: {e}")
        return None